...
import sys
import struct
from constants import OPERATION, REGISTER
...
class AssemblyEncoder:
    ...
    def _import_data(self, path):
        self._lines = [line.strip().replace(',', ' ').lower().split() for line in open(path)]
    ...
    def _analyze_per_line(self, pos):
        ...
        if operation[1] == 0:
            ...
            code |= operation[0] << 26
            if data[1] in self._address:
                addr = self._address[data[1]] >> 2
                print(addr)
                code |= addr & 0x3ffffff
            else:
                try:
                    addr = int(self._eval(data[1]))
                    addr = addr >> 2
                    code |= addr & 0x3ffffff
                except NameError:
                    print("ERROR: failed while processing ", pos, " line")
